...
# Y_3280 combined 4 scans and 820 subjects from original data
# w/ dim 1200*116*3280

Y <- Y_3280

# d is random array with dimension (7,116,3280)
# u is random matrix with dimension (33,p) where p=2 in this case
# v is random array with dimension (p,116,3280) where p=2

p <- 2
d <- array(data = runif(7*116*3280), dim = c(7,116,3280))
u <- matrix(data = runif(33*p), nrow = 33, ncol = p)
v <- array(data = runif(p*116*3280), dim = c(p,116,3280))
...
lr.func.stp12 <- function(Y, D, d, B, u, v, lambda, R){
  ...
  for (iter in 1:maxIter)
  {
    ...
    # For previous, d is random array with dimension (7,116,3280)
    d = array(data = sol, dim = c(7,116,3280))
    for (n in 1:N) {
      d[,,n] = H2%*%Y[,,n]
      d[,,n] = d[,,n] - H2%*%B%*%u%*%v[,,n]
    }
...
# d is random array with dimension (7,116,3280)
# out.d is matrix with dimension (7,116*3280)
out.d <- matrix(data = output1$d, ncol = 116*3280)
image(out.d)
